...
const Header = (props) => {
  return (
    <header className="w-100 navbar navbar-dark bg-dark text-light justify-content-around pt-3">
      ...
      <div role="button" id="devcarrinho" className="position-relative d-flex align-items-center justify-content-center gap-2">
        <i className="bi bi-cart4 accordion text-light fs-2" data-bs-toggle="offcanvas" data-bs-target="#carrinhoOffCanvas"></i>
        {props.contadorJogos > 0 && (<span className="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">{props.contadorJogos}
        </span>)}
      </div>
    </header>
  );
